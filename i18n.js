const I18N = {
  ...
  updateJS() {
    ...
    // (show3DMsg uses tx() now so it picks correct lang automatically)

    // ── IW LANGUAGE SELECTOR label ──
    const iwLangLabel = document.querySelector('#iwLangSelector + div + input');
    const iwLangTitle = document.querySelector('[for="iwLangSelector"], label[data-for="iwLangSelector"]');

    // ── MAIN HISTORY SYSTEM — translatable ──
    // Refresh open History pages when language switches
    if(!document.getElementById('historyMainPage')?.classList.contains('hidden')) {
      if(typeof switchHistTab === 'function'){
        const activeTab = document.getElementById('histTabQR')?.style.borderColor?.includes('a855f7') ? 'qr' : 'home';
        switchHistTab(activeTab);
      }
    }
    if(!document.getElementById('histCategoryPage')?.classList.contains('hidden')) {
      if(typeof openHistCategory === 'function' && typeof _histCurrentCategory !== 'undefined' && _histCurrentCategory) {
        openHistCategory(_histCurrentCategory);
      }
    }
    // History Delete buttons
    const histDelAll = document.getElementById('histCatDeleteBtn');
    if(histDelAll) histDelAll.textContent = this.t('hist_delete_all');
    const histDelItem = document.getElementById('histDetailDeleteBtn');
    if(histDelItem) histDelItem.textContent = this.t('hist_delete_btn');
