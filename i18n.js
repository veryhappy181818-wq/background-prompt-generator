const I18N = {
  ...
  updateJS() {
    ...
    // ── CATEGORY PAGE — translatable ──
    const catTitle = document.getElementById('catPageTitle');
    if(catTitle) catTitle.textContent = this.t('cat_title');

    const catSub = document.getElementById('catPageSub');
    if(catSub) catSub.textContent = this.t('cat_sub');

    const catConfirmBtn = document.getElementById('catConfirmBtn');
    if(catConfirmBtn) catConfirmBtn.textContent = this.t('cat_confirm');

    // Re-render category grid if open (so category names update in correct language)
    if(typeof renderCatGrid === 'function' && document.getElementById('categoryPage')?.style.display !== 'none') {
      renderCatGrid();
      if(typeof updateCatUI === 'function') updateCatUI();
    }

    // Re-render all category banners in new language
    if(typeof showCategoryBanners === 'function' && typeof _userCats !== 'undefined' && _userCats.length) {
      showCategoryBanners();
    }
