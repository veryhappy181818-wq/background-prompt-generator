const I18N = {
  ...
  async load(lang) {
    ...
    try {
      ...
      if (lang !== 'hi') return await this.load('hi');
      return {};
    }
  },

  t(key) {
    const dict = this._cache[this._lang] || this._cache['hi'] || {};
    return dict[key] || this._cache['hi']?.[key] || key;
  },
  ...
  async init() {
    const saved = localStorage.getItem('ns_lang') || 'hi';
    await this.load('hi');
